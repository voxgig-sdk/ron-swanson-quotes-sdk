...
import (
	...
	omni "github.com/voxgig-sdk/ron-swanson-quotes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RonSwansonQuotesError); is {
				out := map[string]any{
					"name":    "RonSwansonQuotesError",
...
func omniCtx(arg any, client *sdk.RonSwansonQuotesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RonSwansonQuotesSDK); is {
			client = live
