...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("RON_SWANSON_QUOTES_TEST_LIVE") == "TRUE" ||
		os.Getenv("RON_SWANSON_QUOTES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("RON_SWANSON_QUOTES_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.RonSwansonQuotesSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.RonSwansonQuotesSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.RonSwansonQuotesError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.RonSwansonQuotesSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.RonSwansonQuotesError{Msg: msg, Code: code}
}
