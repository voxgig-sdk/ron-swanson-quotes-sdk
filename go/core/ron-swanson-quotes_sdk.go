...
import (
	...
	vs "github.com/voxgig-sdk/ron-swanson-quotes-sdk/go/utility/struct"
)

type RonSwansonQuotesSDK struct {
	Mode     string
...
func (sdk *RonSwansonQuotesSDK) OptionsMap() map[string]any {
...
func (sdk *RonSwansonQuotesSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *RonSwansonQuotesSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *RonSwansonQuotesSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *RonSwansonQuotesSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *RonSwansonQuotesSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *RonSwansonQuotesSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("RonSwansonQuotesSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *RonSwansonQuotesSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *RonSwansonQuotesSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("RonSwansonQuotesSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *RonSwansonQuotesSDK {
	...
	sdk := NewRonSwansonQuotesSDK(sdkopts)
	sdk.Mode = "test"
