...
//     (\`new RonSwansonQuotesSDK(...)\` / \`RonSwansonQuotesSDK.test(...)\` become
//     \`RonSwansonQuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RonSwansonQuotesSDK'
...
const TEST_SEED = {"entity":{"quote":{"test01":{"id":"test01"}},"schema":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RonSwansonQuotesSDK(...)\` and any
// \`RonSwansonQuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RonSwansonQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RonSwansonQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
