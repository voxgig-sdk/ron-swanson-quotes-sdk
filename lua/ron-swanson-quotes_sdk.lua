...
require("ron-swanson-quotes_types")
...
local RonSwansonQuotesSDK = {}
RonSwansonQuotesSDK.__index = RonSwansonQuotesSDK
...
RonSwansonQuotesSDK._make_feature = _make_feature


function RonSwansonQuotesSDK.new(options)
  local self = setmetatable({}, RonSwansonQuotesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function RonSwansonQuotesSDK:options_map()
...
function RonSwansonQuotesSDK:get_utility()
  return Utility.copy(self._utility)
end


function RonSwansonQuotesSDK:get_root_ctx()
  return self._rootctx
end


function RonSwansonQuotesSDK:prepare(fetchargs)
...
function RonSwansonQuotesSDK:direct(fetchargs)
...
function RonSwansonQuotesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function RonSwansonQuotesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "RonSwansonQuotesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function RonSwansonQuotesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function RonSwansonQuotesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "RonSwansonQuotesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function RonSwansonQuotesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = RonSwansonQuotesSDK.new(sdkopts)
  sdk.mode = "test"
