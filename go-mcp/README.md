# ron-swanson-quotes-mcp

[MCP](https://modelcontextprotocol.io) server exposing the RonSwansonQuotes SDK as
two agent tools — `ron-swanson-quotes_list` and `ron-swanson-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ron-swanson-quotes-mcp)
make build

# 2. Provide credentials via the environment
export RON_SWANSON_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ron-swanson-quotes \
  -- /absolute/path/to/ron-swanson-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ron-swanson-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ron-swanson-quotes_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// ron-swanson-quotes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ron-swanson-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ron-swanson-quotes \
     -- "$PWD"/dist/*/ron-swanson-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ron-swanson-quotes_list` and `ron-swanson-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list quote using ron-swanson-quotes"*
   and it calls `ron-swanson-quotes_list` with `{"entity":"quote"}`.
...
```sh
export RON_SWANSON_QUOTES_APIKEY=sk_live_xxx            # API key
export RON_SWANSON_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ron-swanson-quotes_list` tool
...
### Call the `ron-swanson-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ron-swanson-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ron-swanson-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RON_SWANSON_QUOTES_APIKEY` | API key sent with every request. |
| `RON_SWANSON_QUOTES_BASE` | Optional override of the API base URL. |
...
```sh
./ron-swanson-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ron-swanson-quotes_load","arguments":{"entity":"quote","query":{"id":1}}}}'
```
