...
boru-driven command-line client **and** interactive REPL for the RonSwansonQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ron-swanson-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./ron-swanson-quotes-cli --help

# 3. Provide credentials once, via the environment
export RON_SWANSON_QUOTES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ron-swanson-quotes-cli list quote
./ron-swanson-quotes-cli load 1 quote            # {id:1} shorthand
./ron-swanson-quotes-cli load '{id:1}' quote       # explicit match map
./ron-swanson-quotes-cli list schema

# 5. Override the API base URL for a single call
RON_SWANSON_QUOTES_BASE=https://api.example.com ./ron-swanson-quotes-cli list quote

# 6. No arguments -> interactive REPL
./ron-swanson-quotes-cli
ron-swanson-quotes> list quote
ron-swanson-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ron-swanson-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ron-swanson-quotes-cli list quote
   ```
...
```sh
./ron-swanson-quotes-cli load 1 quote          # scalar shorthand for {id:1}
./ron-swanson-quotes-cli load '{id:1}' quote     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `ron-swanson-quotes>`). Each line is
evaluated as its own boru expression:

```text
$ ./ron-swanson-quotes-cli
ron-swanson-quotes> list quote
ron-swanson-quotes> /help
ron-swanson-quotes> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ron-swanson-quotes-cli`. |
...
(`./ron-swanson-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
