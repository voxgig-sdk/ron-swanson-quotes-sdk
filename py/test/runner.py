# RonSwansonQuotes SDK test runner
...
class RonSwansonQuotesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RonSwansonQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RonSwansonQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RonSwansonQuotesTestRunner.getenv("RON_SWANSON_QUOTES_TEST_LIVE")
        override = RonSwansonQuotesTestRunner.getenv("RON_SWANSON_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RonSwansonQuotesTestRunner.getenv(key)
        ...
        explain = RonSwansonQuotesTestRunner.getenv("RON_SWANSON_QUOTES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["RON_SWANSON_QUOTES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if RonSwansonQuotesTestRunner._test_control is not None:
            return RonSwansonQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RonSwansonQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RonSwansonQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RonSwansonQuotesTestRunner.load_test_control()
...
def load_env_local():
    RonSwansonQuotesTestRunner.load_env_local()


def env_override(m):
    return RonSwansonQuotesTestRunner.env_override(m)


def entity_data(v):
    return RonSwansonQuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RonSwansonQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RonSwansonQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RonSwansonQuotesTestRunner.load_test_control()


def live_client_options():
    return RonSwansonQuotesTestRunner.live_client_options()


def live_delay_ms():
    return RonSwansonQuotesTestRunner.live_delay_ms()
