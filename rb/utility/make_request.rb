...
module RonSwansonQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RonSwansonQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = RonSwansonQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RonSwansonQuotesResponse.new(fetched)
    else
