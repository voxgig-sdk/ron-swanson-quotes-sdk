...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = RonSwansonQuotesOmni.make_runner(TEST_JSON_FILE, RonSwansonQuotesSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    RonSwansonQuotesError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : RonSwansonQuotesOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = RonSwansonQuotesOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
