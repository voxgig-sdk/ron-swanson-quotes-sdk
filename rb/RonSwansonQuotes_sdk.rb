...
require_relative 'RonSwansonQuotes_types'
...
class RonSwansonQuotesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RonSwansonQuotesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RonSwansonQuotesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RonSwansonQuotesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RonSwansonQuotesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RonSwansonQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RonSwansonQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RonSwansonQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RonSwansonQuotesError.new(
        "#{op}_allow",
        "RonSwansonQuotesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue RonSwansonQuotesError => err
    ...
    if fetched.is_a?(Hash)
      status = RonSwansonQuotesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RonSwansonQuotesError.new(
        "graphql_error", "RonSwansonQuotesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RonSwansonQuotesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
