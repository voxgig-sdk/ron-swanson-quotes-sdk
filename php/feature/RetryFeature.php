...
// RonSwansonQuotes SDK retry feature
...
class RonSwansonQuotesRetryFeature extends RonSwansonQuotesBaseFeature
{
    ...
    public function init(RonSwansonQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RonSwansonQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RonSwansonQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RonSwansonQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
