...
// RonSwansonQuotes SDK feature test
//
...
class FtCtrl extends RonSwansonQuotesControl
{
...
class FtClient
{
    ...
    private RonSwansonQuotesUtility $utility;

    public function __construct(RonSwansonQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): RonSwansonQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RonSwansonQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RonSwansonQuotesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RonSwansonQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RonSwansonQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(RonSwansonQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RonSwansonQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RonSwansonQuotesResponse($fetched);
        }
    ...
    private function fail(RonSwansonQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RonSwansonQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
