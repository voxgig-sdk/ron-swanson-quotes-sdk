...
// RonSwansonQuotes SDK pipeline test
//
...
require_once __DIR__ . '/../ronswansonquotes_sdk.php';
...
{
    private static function utility(): RonSwansonQuotesUtility
    {
        return new RonSwansonQuotesUtility();
    }
    ...
    private static function ctx(array $over = []): RonSwansonQuotesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RonSwansonQuotesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RonSwansonQuotesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RonSwansonQuotesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RonSwansonQuotesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RonSwansonQuotesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RonSwansonQuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RonSwansonQuotesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RonSwansonQuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RonSwansonQuotesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RonSwansonQuotesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RonSwansonQuotesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RonSwansonQuotesResponse([]),
            'result' => new RonSwansonQuotesResult([]),
        ]);
        [, $err] = RonSwansonQuotesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RonSwansonQuotesSpec([]),
            'response' => null,
            'result' => new RonSwansonQuotesResult([]),
        ]);
        [, $err] = RonSwansonQuotesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RonSwansonQuotesSpec(['step' => 's']),
            'response' => new RonSwansonQuotesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RonSwansonQuotesResult([]),
        ]);
        [, $err] = RonSwansonQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RonSwansonQuotesSpec(['step' => 's']),
            'response' => new RonSwansonQuotesResponse(self::resp(200, ['v' => 1])),
            'result' => new RonSwansonQuotesResult([]),
        ]);
        [, $err] = RonSwansonQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RonSwansonQuotesSpec(['step' => 's']),
            'response' => new RonSwansonQuotesResponse(self::resp(200, ['v' => 2])),
            'result' => new RonSwansonQuotesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RonSwansonQuotesMakeResponse::call($ctx);
    ...
    {
        $preset = new RonSwansonQuotesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RonSwansonQuotesSpec([]),
            'response' => new RonSwansonQuotesResponse([]),
            'result' => new RonSwansonQuotesResult([]),
        ...
        [$got, $err] = RonSwansonQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RonSwansonQuotesResult([])]);
        [, $err] = RonSwansonQuotesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RonSwansonQuotesSpec([]), 'result' => null]);
        [, $err] = RonSwansonQuotesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RonSwansonQuotesSpec(['step' => 's']),
            'result' => new RonSwansonQuotesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RonSwansonQuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RonSwansonQuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RonSwansonQuotesSpec(['step' => 's']),
            'result' => new RonSwansonQuotesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RonSwansonQuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RonSwansonQuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RonSwansonQuotesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RonSwansonQuotesSpec([]),
            'result' => new RonSwansonQuotesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RonSwansonQuotesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RonSwansonQuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RonSwansonQuotesError('boom', 'boom');
        $utility->fetcher = function (RonSwansonQuotesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RonSwansonQuotesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RonSwansonQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RonSwansonQuotesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RonSwansonQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RonSwansonQuotesResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RonSwansonQuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RonSwansonQuotesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RonSwansonQuotesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RonSwansonQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RonSwansonQuotesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RonSwansonQuotesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RonSwansonQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RonSwansonQuotesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RonSwansonQuotesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RonSwansonQuotesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RonSwansonQuotesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RonSwansonQuotesResult(['ok' => false])]);
        $this->expectException(RonSwansonQuotesError::class);
        RonSwansonQuotesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RonSwansonQuotesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RonSwansonQuotesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RonSwansonQuotesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RonSwansonQuotesBaseFeature();
        $b = new RonSwansonQuotesBaseFeature();
        RonSwansonQuotesFeatureAdd::call($ctx, $a);
        RonSwansonQuotesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RonSwansonQuotesBaseFeature
    {
        $f = new RonSwansonQuotesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RonSwansonQuotesFeatureAdd::call($ctx, self::named_feature('a'));
        RonSwansonQuotesFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        RonSwansonQuotesFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        RonSwansonQuotesFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        RonSwansonQuotesFeatureAdd::call($ctx, $replace);
        ...
        RonSwansonQuotesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RonSwansonQuotesContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RonSwansonQuotesSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RonSwansonQuotesPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RonSwansonQuotesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RonSwansonQuotesResponse(['headers' => null]),
            'result' => new RonSwansonQuotesResult([]),
        ]);
        RonSwansonQuotesResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        RonSwansonQuotesResultBody::call($ctx);
