...
//    only - a typed RonSwansonQuotesContext there would make every ctx
...
final class RonSwansonQuotesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new RonSwansonQuotesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new RonSwansonQuotesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new RonSwansonQuotesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new RonSwansonQuotesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new RonSwansonQuotesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?RonSwansonQuotesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RonSwansonQuotesError(is_string($code) ? $code : '', $msg);
    }
